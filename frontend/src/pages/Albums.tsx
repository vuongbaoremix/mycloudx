...
export default function Albums() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-10 md:py-16">
      ...
      {/* Header */}
      <div className="mb-12">
        <h1 className="text-4xl md:text-5xl font-extrabold font-headline text-slate-900 dark:text-white tracking-tight mb-2">Album của tôi</h1>
        <p className="text-lg md:text-xl text-slate-500 dark:text-slate-400 font-medium">Câu chuyện của bạn, được sắp xếp gọn gàng.</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-8 md:gap-12">
        ...
        <div 
          ...
          className="group relative aspect-square rounded-[2.5rem] border-[3px] border-dashed border-blue-200/60 dark:border-blue-500/30 bg-blue-50/50 dark:bg-blue-500/5 hover:bg-blue-100/50 dark:hover:bg-blue-500/10 transition-all cursor-pointer flex flex-col items-center justify-center shadow-sm hover:shadow-xl"
        >
          <div className="w-16 h-16 rounded-full bg-white dark:bg-surface shadow-[0_8px_30px_rgba(0,0,0,0.08)] border border-blue-100 dark:border-blue-500/20 flex items-center justify-center text-blue-600 dark:text-blue-400 transition-transform duration-500 group-hover:scale-110 group-hover:shadow-[0_8px_30px_rgba(59,130,246,0.2)] mb-4">
            <Plus size={28} strokeWidth={2.5} />
          </div>
          <span className="text-blue-700 dark:text-blue-400 font-semibold tracking-tight text-lg">Tạo Album mới</span>
        </div>
          ...
          return (
            ...
            >
              ...
              <div className="relative aspect-square mb-5 w-full">
                {/* Bottom layer */}
                <div className={`absolute inset-0 bg-surface-container rounded-[2.5rem] overflow-hidden 
                                shadow-sm transition-transform duration-500 ease-out 
                                group-hover:translate-x-6 group-hover:-translate-y-4 group-hover:rotate-6 origin-bottom-left ${!backSrc ? 'mix-blend-multiply opacity-60' : ''}`}>
                ...
                {/* Middle layer */}
                <div className={`absolute inset-0 bg-surface-container-high rounded-[2.5rem] overflow-hidden 
                                shadow-md transition-transform duration-500 ease-out 
                                group-hover:-translate-x-5 group-hover:-translate-y-2 group-hover:-rotate-3 origin-bottom-right z-10 ${!middleSrc ? 'mix-blend-multiply opacity-80' : ''}`}>
                ...
                <div className={`absolute inset-0 z-20 rounded-[2.5rem] shadow-[0_10px_40px_rgba(0,0,0,0.12)] 
                                overflow-hidden transition-transform duration-500 ease-out group-hover:-translate-y-2 
                                ${hasPreview ? 'bg-surface' : 'bg-gradient-to-br ' + grad} flex items-center justify-center border-4 border-white/40 dark:border-white/10`}
                >
                  {coverSrc ? (
                    ...
                    <Images size={64} className="text-white/40 mix-blend-overlay drop-shadow-md" />
                  )}
                  
                  {/* Delete button (only visible on hover over main layer) */}
                  <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    <button
                      className="w-10 h-10 rounded-full bg-black/20 hover:bg-red-500/90 backdrop-blur-md text-white flex items-center justify-center shadow-lg transition-colors border border-white/20"
              ...
              {/* Album Details */}
              <div className="px-2">
                <h3 className="text-xl font-bold font-headline text-on-surface mb-1.5 truncate transition-colors group-hover:text-primary">
                  {album.name}
                </h3>
                <div className="flex items-center text-sm font-semibold text-on-surface-variant tracking-wide">
                  <Images size={14} className="mr-2 opacity-70" /> 
